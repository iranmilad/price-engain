...
class PriceSeries:
    ...
    def stockprice(self,bars,years,symbol):
        ...
        if len(years)==0 or len(bars)==0:
            ...
            self.append_new_line('snapshotForex.txt', symbol)
        else:
    ...
    def update_bar(self):
        ...
        exist_forex_symbol= list(set(clasters).intersection(set(self.forex_symbol)) )
        for i in range(0,len(exist_forex_symbol),100):
            claster = exist_forex_symbol[i:i + 100]
            ...
            if claster:
                contracts=self.getContractForex(claster)
                self.threads=[]
