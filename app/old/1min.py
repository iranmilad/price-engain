...
api_key="Hnt7HPS4RjJehhPRr6VqUTzOr5hNsCBOfBpCQs5LlXueAE7wzYce0QUNPAHEgoo0"
api_secret="SKeHvrGe3Sbok35e4G4U9PEK5cIo1raKXmIJxRCZTOGatmuy6m84aRWWBrhrzEU2"
...
def crypto_price(bars,symbol):

    r = redis.Redis(host='149.28.120.38', port=6379,password='6$gtA453', db=1)
...
crypto_client = Client(api_key, api_secret)
ib = IB()

client=51
ib.connect('127.0.0.1', 7497, clientId=client,timeout=10)
claster=getSymbols('1min')

for symbol in claster:
    contract=getContract(symbol)
    if symbol in crypto_symbol:
        bars = contract.get_historical_klines(symbol, Client.KLINE_INTERVAL_1MINUTE, "1 day ago UTC")
        crypto_price(bars,symbol)
    elif symbol in forex_symbol:
        candelShow='MIDPOINT'
        bars = ib.reqHistoricalData(
            contract, endDateTime='', durationStr='1 D',
            barSizeSetting='1 min', whatToShow=candelShow, useRTH=True)
    else:
        candelShow='TRADES'
